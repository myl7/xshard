...
if not SKIP_COPY:
    for i, ip in enumerate(ips[0]):
        subprocess.run(
            RSCP_CMD + f'../cmd/mingchain-leader/mingchain-leader gcfg.json {i}.bin ubuntu@{ip}:/home/ubuntu/', shell=True)
        print(f'Leader {i} is copied')
    for i in range(4):
        shard_ips = ips[i + 1]
        for j, ip in enumerate(shard_ips):
            subprocess.run(
                RSCP_CMD + f'../cmd/mingchain-validator/mingchain-validator gcfg.json {str(i) + ".bin"} ubuntu@{ip}:/home/ubuntu/', shell=True)
